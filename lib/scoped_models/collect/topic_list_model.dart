...
class TopicListModel extends BaseModel {
  List<TopicInfo> topics = List<TopicInfo>();
...
//      "projectId":stateModel.defaultProjectId,
    ...
    HttpUtil.post(isBulkCollectPage?HttpOptions.findMyCollectTopicPage:HttpOptions.findCustomerTopicPage, _getListCallBack,
        jsonData: json.encode(params), errorCallBack: _getListErrorCallBack);
  }
  _getListCallBack(data) {
    try {
      TopicListResponse infos = TopicListResponse.fromJson(data);
      if (infos.success()) {
...
//        }
  ...
  setAllCollected(bool checked){
    ...
    if(checked){
      if(topics!=null){
        collectCheckedList.addAll(topics.map((info)=>info.topicId).toList());
      }
    }
    notifyListeners();
  }
  ...
  cancelCollectList(List<int> collectIdList,int type,Function callback) {
    ...
    params['collectType'] = "2";//话题我的收藏
    HttpUtil.post(HttpOptions.collectTopic, (data){
  ...
  static TopicListModel of(context) =>
      ScopedModel.of<TopicListModel>(context);
}
