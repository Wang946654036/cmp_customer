...
class CanBookRoomList extends StatefulWidget {
  ReservationModel _model;


  CanBookRoomList(this._model,);
...
class _CanBookRoomListState extends State<CanBookRoomList> {
                                        ...
                                        Navigate.toNewPage(
                                          ...
                                          if (widget._model.result_fail ==
                                            ...
                                            Navigate.toNewPage(
                                              ...
                                              if(widget._model.result_suc_create==requestCodeInside){
                                                //回去刷新
...
//刷新操作
  _refresh() {
    ...
    widget._model.canBookHandleRefresh(param, expandStateList,preRefresh: true);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return CommonScaffold(
      appTitle: '我要预定',
      popBack: () {
        if ((widget._model.saveInfoList?.length ?? 0) > 0) {
          Navigate.toNewPage(ReservationCheckPage(widget._model)); //确认预定列表
        }
           ...
           GestureDetector(
              onTap: () {
                CommonPicker.datePickerModal(context, onConfirm: (String date) {
                  if (date != null)
                  if(StringsHelper.isNotEmpty(date)&&DateTime.parse(date).isBefore(DateTime.parse(StringsHelper.formatterYMD.format(DateTime.now())))){
      CommonToast.show(type: ToastIconType.INFO,msg:"不能选早于今天的时间");
      }else
                    setState(() {
              ...
              child: Container(
                ...
                child: Row(mainAxisAlignment:MainAxisAlignment.center,crossAxisAlignment: CrossAxisAlignment.center,children: <Widget>[CommonText.text15('预约日期：$surrenderTime',textAlign: TextAlign.center,
                    color: UIData.darkGreyColor),Icon(
  ...
  bool isSaved(MeetingRoomInfo info) {
    bool isSaved = false;
    if (widget._model.saveInfoList != null&&info!=null) {
      for (int i = 0; i < widget._model.saveInfoList.length; i++) {
        if (widget._model.saveInfoList[i].roomId == info.roomId&&widget._model.saveInfoList[i].orderDate==surrenderTime) {
          isSaved = true;
          return isSaved;
        }
      }
