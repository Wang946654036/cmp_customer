import 'package:cmp_customer/ui/home/navigation_item.dart';
import 'package:flutter/material.dart';

enum ACEBottomNavigationBarType { normal, zoom, zoomout, zoomoutonlypic, protruding }
...
class _ACEBottomNavigationBar extends State<ACEBottomNavigationBar> with TickerProviderStateMixin, RouteAware {
  ...
  _setSelected(UniqueKey key) {
    if (mounted) {
      setState(() {
        curSelectedIndex = widget.items.indexWhere((tabData) => tabData.key == key);
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return Stack(alignment: Alignment.bottomCenter, overflow: Overflow.visible, children: <Widget>[
      Container(
          height: 60.0,
          ...
          child: Row(
              ...
              children: widget.items
                  .map((item) => NavigationItem(
                      ...
                      textSelectedColor:
                          (item.textSelectedColor == null) ? this.textSelectedColor : item.textSelectedColor,
                      textUnSelectedColor:
                          (item.textUnSelectedColor == null) ? this.textUnSelectedColor : item.textUnSelectedColor,
                      iconSelectedColor:
                          (item.iconSelectedColor == null) ? this.iconSelectedColor : item.iconSelectedColor,
                      iconUnSelectedColor:
                          (item.iconUnSelectedColor == null) ? this.iconUnSelectedColor : item.iconUnSelectedColor,
                      ...
                      callbackFunction: (uniqueKey) {
                        int selected = widget.items.indexWhere((tabData) => tabData.key == uniqueKey);
  ...
  Widget protrudingWid() {
    Widget proWid;
    if (widget.items.length % 2 == 0 || widget.type != ACEBottomNavigationBarType.protruding) {
...
//                                    : Colors.white),
                            child: Padding(
                                ...
                                child: protrudingItemWid(widget.items[protrudingIndex]))))
                  ]))));
    }
    return proWid;
  }
