...
class DecorationNodeListView extends StatefulWidget {
  List<NodeList> recordList;

  DecorationNodeListView(this.recordList);
...
class _NodeListViewState extends State<DecorationNodeListView> {
...
//                                      )
                              ...
                              Visibility(
                                ...
                                child: CommonText.text13(node.content ?? "",
  ...
  List<WorkOtherProcessNode> tran2WorkOtherProcessNodeList() {
    ...
    for (int i = 0; i < widget.recordList.length; i++) {
      Map map = getWorkOtherOperationStateType(widget.recordList[i].operation);
...
//      int uiType =StringsHelper.isNotEmpty(widget.recordList[i].creatorType)?widget.recordList[i].creatorType=='CUST'?1:0:-1;
      String time = widget.recordList[i].createDate ?? '至今';

      StringBuffer content = new StringBuffer();
      if (widget.recordList[i] != null) {
        if (StringsHelper.isNotEmpty(widget.recordList[i].operationResult)) {
          String passStr = widget.recordList[i].operationResult;
          content.write(passStr + '\n');
        }
        if (StringsHelper.isNotEmpty(widget.recordList[i].other)) {
          String handoverTime = widget.recordList[i].other;
          content.write(handoverTime + '\n');
        }
        if (StringsHelper.isNotEmpty(widget.recordList[i].remark)) {
          String processContent = widget.recordList[i].remark;
      ...
      WorkOtherProcessNode workOtherProcessNode = new WorkOtherProcessNode(
          ...
          uiType: 1,
          time: time,
          name: widget.recordList[i].operationName,
...
//          photoList: workOrderPhotoList,
  ...
  Map<String, dynamic> getWorkOtherOperationStateType(String operation) {
