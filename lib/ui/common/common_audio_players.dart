...
class CommonAudioPlayers extends StatefulWidget {
  String uuid; //服务器中音频文件的uuid
  CommonAudioPlayers(this.uuid);

  @override
  _CommonAudioPlayersState createState() => new _CommonAudioPlayersState(uuid);
}

class _CommonAudioPlayersState extends State<CommonAudioPlayers> {
  Duration duration;
  Duration position;
  String playUrl;
  String uuid;
...
//      position != null ? _getRemainingTime() : '';
  ...
  _CommonAudioPlayersState(this.uuid);
  ...
  void initAudioPlayer() {
    ...
    audioPlayer.onDurationChanged.listen((Duration p)=> duration = p);
...
//        audioPlayer.onAudioPositionChanged.listen((Duration p) => setState(() => position = p), onError: (msg) {
//      setState(() {
//        playerState = PlayerState.stopped;
//        position = new Duration(seconds: 0);
//      });
            ...
            children: <Widget>[
              Padding(
                  padding: EdgeInsets.only(
                      left: ScreenUtil.getInstance().setWidth(10), right: ScreenUtil.getInstance().setWidth(5)),
                  child: Image(
