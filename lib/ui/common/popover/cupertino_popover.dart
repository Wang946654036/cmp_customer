...
import 'package:cmp_customer/ui/common/popover/widget_util.dart';
...
class CupertinoPopover extends StatefulWidget {
  ...
  CupertinoPopover({
    ...
    BoxConstraints temp;
    if(constraints != null){
      temp = BoxConstraints(maxHeight:223.0,maxWidth:150.0).copyWith(
      ...
      temp=BoxConstraints(maxHeight:223.0,maxWidth:150.0);
    }
...
class _CupertionPopoverPositionRenderObject extends RenderShiftedBox{
  ...
  Offset calcOffset(Size size){
    ...
    var isArrowUp = MediaQueryData.fromWindow(ui.window).size.height > attachRect.bottom + size.height + CupertinoPopoverState._arrowHeight;

    if(attachRect.left > size.width / 2 &&
        MediaQueryData.fromWindow(ui.window).size.width - attachRect.right > size.width / 2){ //判断是否可以在中间
      bodyLeft = attachRect.left +  attachRect.width / 2 - size.width / 2;
    }else if(attachRect.left < size.width / 2){ //靠左
      bodyLeft = 10.0;
    }else{ //靠右
      bodyLeft = MediaQueryData.fromWindow(ui.window).size.width - 10.0 - size.width;
    }
